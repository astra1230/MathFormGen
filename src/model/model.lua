...
function model:step(batch, forward_only, beam_size, trie)
    ...
    local feval = function(p) --cut off when evaluate
        ...
        if forward_only then
            -- final decoding
            for p=beam_size,1,-1 do
                
                local labels = localize(torch.zeros(batch_size, target_l)):fill(1)
                print(self.beam_scores)

                local scores, indices = torch.max(self.beam_scores[{{1,batch_size},{p}}], 2) -- batch_size, 1
--                 local scores = self.beam_scores[{{1,batch_size},{p}}] -- batch_size, 1
                indices=localize(torch.zeros(batch_size, 1)):fill(p)

                print(scores)
                print(indices)
    --             indices=localize(torch.zeros(batch_size, 1)):fill(4)
    --             local scoress, indicess = torch.sort(self.beam_scores[{{1,batch_size},{}}],2) -- batch_size, 1
    --             scores=scoress[{{1,batch_size},{1}}]
    --             indices=indicess[{{1,batch_size},{1}}]
    --             local scores, indices = torch.kthvalue(self.beam_scores[{{1,batch_size},{}}],1, 2) -- batch_size, 1
    --             local scores, indices = torch.kthvalue(self.beam_scores[{{1,batch_size},{}}],2, 2) -- batch_size, 1
    --             local scores, indices = torch.kthvalue(self.beam_scores[{{1,batch_size},{}}],3, 2) -- batch_size, 1
    --             local scores, indices = torch.kthvalue(self.beam_scores[{{1,batch_size},{}}],4, 2) -- batch_size, 1

                indices = localize(indices:double())
                scores = scores:view(-1) -- batch_size
                indices = indices:view(-1) -- batch_size
    --             print(self.current_indices_history)
